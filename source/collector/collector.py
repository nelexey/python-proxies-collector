...
async def parser2():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        nonce = re.search(regex, await fetch(session, url), re.MULTILINE).group(1)

        url = f'https://proxycompass.com/wp-admin/admin-ajax.php?action=proxylister_download&nonce={nonce}&format=json&filter={'{}'}'
...
async def collect(count: int | None = None, filename: str = 'proxies_list', frmt='json'):
    ...
    async def log_proxies(sitename, working, all_proxies):
        print(f'{sitename:20} | {working}/{all_proxies} working proxies...')
    ...
    if count is not None:
        for name, parser in sites_parsers.items():
            ...
            try:
                ...
                print('Error while parsing, skip...')
    else:
        for name, parser in sites_parsers.items():
            ...
            try:
                ...
                print('Error while connecting to server, skip...')
